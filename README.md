...
js/                   app logic (vanilla JS, no framework)
  ...
  worldmap.js                 pure Reference-map helpers (ISO lookup, grouping, sorting)
...
data/
  ...
  worldmap.js              country outlines for the Reference map (generated — don't hand-edit)
scripts/
  ...
  build-worldmap.mjs       generates data/worldmap.js from Natural Earth (maintainer tool)
...
Study Mode's Reference tab has a world map you can click to filter the pool by
country. Its outlines live in `data/worldmap.js`, generated by
`scripts/build-worldmap.mjs` from [Natural Earth](https://www.naturalearthdata.com/)
...
Natural Earth is public domain — no attribution required — which is why it's
the source. The two obvious off-the-shelf alternatives both lost: SimpleMapLab's
CC0 world SVG is 2.6 MB with zero `id`/`class` attributes on its 242 paths
(nothing to attach a click handler to), and `flekschas/simple-world-map` is
74 KB with ISO ids but CC BY-SA 3.0, carrying attribution *and* ShareAlike
obligations.

The generated file is ~51 KB — smaller than `data/mics.js`. The projection is
equirectangular, which is plain arithmetic, so the script needs no npm
dependency (node builtins only, like every other script here). Coordinates are
rounded to integers and emitted as relative linetos, and the `viewBox` is
cropped to the actual content bounds since Antarctica is dropped.

You only need to re-run this if you want different source data or projection
settings — it doesn't depend on `data/mics.js`. Adding a mic from a **new
country** does need one manual step though: add that country to `COUNTRY_ISO`
in `js/worldmap.js`, which bridges `data/mics.js`'s colloquial names ("USA",
"UK") to the map's ISO codes. Miss it and the app logs a console warning and
renders that country's chip without highlighting it on the map.
...
- **Reference** — search or browse the full pool and see a mic's complete
  ...
  It also has a world map: click a country (or its chip) to list that
  country's mics, grouped by manufacturer. Columns sort by clicking a header
  on desktop, or a sort pill at any width — the pills exist because below
  800px the board switches to card mode and `.board-row--header` is
  `display: none`, so there are no headers left to click. Sorting by
  Manufacturer groups; every other column flattens to one list, since you
  can't group by maker and order globally by year at the same time.

  The **chips are the real control and the map is a picture that happens to
  be clickable.** At 375px Denmark renders about 5×3 CSS px and Austria 8×3 —
  far under any usable tap target, and their centroids sit close enough that
  enlarged hit areas would overlap and steal each other's clicks. So the
  `<svg>` is `aria-hidden` with nothing focusable inside it, and every bit of
  state it shows is reachable from the chips.

  Unlike the other three tabs, Reference draws from `SELECTABLE_MICS`
  (`retired !== true`) rather than `eligibleMics()` (`needsVerification !== true`),
  so the map, the table and the search box agree with each other. Both
  predicates return 118 today, but only because one mic happens to carry both
  flags — they'd diverge the moment a mic is retired but verified, or
  quarantined but still live.
